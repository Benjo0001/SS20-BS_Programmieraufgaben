...
public class RdmNumAlgo_RandomRnd {
   public static void main(String[] args) {
    ...
    for (int i = 0; i < oneMioRndArray.length; i++) {
        ...
        oneMioRndArray[i] = (int)(rnd.nextDouble() * ((1000000) + 1));
   ...
   public static int runSearchAlgo_slow(int[] oneMioRndArray, int k) {
        boolean found = false;
        int i=0;
        while (i<oneMioRndArray.length && found) {
            if(oneMioRndArray[i]==k){
                found = true;
                if(found)
                    return i;
   ...
   public static long runNTIMES(int nTimes, int[] oneMioRndArray){
        ...
        for (int i = 0; i < nTimes; i++) {
            //runSearchAlgo(oneMioRndArray, ((int)(Math.random() * ((1000000) + 1))));
            // if(runSearchAlgo_slow(oneMioRndArray, ((int)(Math.random() * ((1000000) + 1)))) == true){
            //     counter++;
            // }
            System.out.print(runSearchAlgo_slow(oneMioRndArray, (int)(rnd.nextDouble() * ((1000000) + 1))) +"  ");
        }
